...
<script>
	function checkForm(){
		...
		var valiPhone=document.getElementById("phone").value;
		var re1 =/^(13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0|1|2|3|5|6|7|8|9])\d{8}$/;
		if(valiPhone==null||valiPhone==""||!re1.test(valiPhone)){
			alert("手机号码格式错误!");
			return false;
		}
		var valiEmail=document.getElementById("email").value;
		var re2=/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;
		if(valiEmail==null||valiEmail==""||!re2.test(valiEmail)){
			alert("邮箱格式错误！");
			return false;
		}
		var phoneVale=document.getElementById("fontPhone").innerHTML;
			if(phoneVale!="邮箱可以使用！"){
				alert(phoneVale);
				return false;
			}
		var emailVale=document.getElementById("fontEmail").innerHTML;
		if(emailVale!="号码可以使用！"){
			alert(emailVale);
			return false;
		}
	...
	function checkPhone(){
		...
		// 3.打开连接
		xhr.open("GET","${pageContext.request.contextPath}/userController/checkPhone?time="+new Date().getTime()+"&phonenumber="+phone,true);
		// 4.发送
		xhr.send(null);
	}
	function checkCode(){
	...
	function checkEmail(){
		...
		// 3.打开连接
		xhr.open("GET","${pageContext.request.contextPath}/userController/checkEmail?time="+new Date().getTime()+"&email="+email,true);
		// 4.发送
		xhr.send(null);
	}
		...
		<div class="span24">
			<div class="wrap">
				<div class="main clearfix">
					...
					<form id="registerForm" action="${ pageContext.request.contextPath }/userController/register"  method="post" novalidate="novalidate" onsubmit="return checkForm();">
						<table>
							<tbody><tr>
								<th>
								...
								<td>
									<input type="text" id="username" placeholder="您的名字！" name="username" class="text" maxlength="20"/>
									<span id="span1"></span>
								</td>
							</tr>
							<tr>
								...
								<td>
									<input type="password" id="password" placeholder="8位以上密码！" name="password" class="text" maxlength="18" />
									<span></span>
								</td>
							</tr>
							...
							<tr>
								...
								<td>
									<input type="text" id="email" name="email" placeholder="您的邮箱！" onblur="checkEmail()" class="text" maxlength="30"/>
									<span id="span2"></span>
								</td>
							</tr>
									<tr>
										...
										<td>
												<input type="text" id="phone" placeholder="11位手机号码！" name="phonenumber" maxlength="11" onblur="checkPhone()" class="text" />
												<span id="span4"></span>
										</td>
									</tr>
								...
								<tr>
									...
									<td>
										<span class="fieldSet">
											<input type="text" id="checkcode" name="checkcode" onblur="checkCode()" class="text captcha" maxlength="5" /><img id="checkImg" class="captchaImage" src="${pageContext.request.contextPath }/authCode" onclick="change()" title="点击更换验证码"/>
										</span>
										<span id="span3"></span>
									</td>
								</tr>
								...
								<td>
									<input id="repassword" type="password" placeholder="再次输入密码！" name="repassword" class="text" maxlength="18" />
								</td>
							</tr>
							<tr>
								<th>&nbsp;
...
<div class="ad">
	...
	<dd>
		自由退换、售后上门
								...
								<dd>
									立即登录即可体验在线购物！
									<a href="${ pageContext.request.contextPath }/loginController/login">立即登录</a>
								</dd>
							</dl>
						</div>
					</form>
...
<div id="_my97DP" style="position: absolute; top: -1970px; left: -1970px;"><iframe style="width: 190px; height: 191px;" src="${ pageContext.request.contextPath }/loginController/regist" frameborder="0" border="0" scrolling="no"></iframe></div></body></html>