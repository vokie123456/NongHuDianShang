...
<script type="text/javascript">
	...
	$(function(){
		$("#menudg").treegrid({
			url:'${pageContext.request.contextPath }/web/menuAction/menuIndex',
			idField:'id',
			treeField:'title',
			loadMsg:'请稍后，数据加载中...............',
			pagination:true,
	...
	function add(){
		url="${pageContext.request.contextPath }/web/menuAction/add";
		$('#fm').form('clear');
		$('#cc').combotree('reload');
	...
	function del(){
		...
		if (row == null) {
			...
			$.ajax({
				url:'${pageContext.request.contextPath }/web/menuAction/delete',
				data:{id:row.id},
	...
	function edit(){
		...
		if (selected == null) {
			...
			var row = $('#menudg').datagrid('getSelected');
			url="${pageContext.request.contextPath }/web/menuAction/update?id="+row.id;
			if (row) {
				$('#fm').form('clear');
		...
		<form id="fm" method="post" novalidate>
			<div class="fitem">
				...
				<select name="pId" id="cc" class="easyui-combotree" style="width:165px;"
				        data-options="url:'${pageContext.request.contextPath }/web/menuAction/queryAllMenus'">
				</select>
			</div>
			<div class="fitem">
