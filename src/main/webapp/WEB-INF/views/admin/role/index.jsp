...
<script type="text/javascript">
	...
	$(function(){
		//初始化角色列表
		$("#roles").datagrid({
			url:'${pageContext.request.contextPath }/web/roleAction/queryRoles',
			idField:'id',
			loadMsg:'请稍后，数据加载中...............',
			pagination:true,
			striped:true,
			...
			onClickRow:function(index,data){
				$.getJSON('${pageContext.request.contextPath }/web/menuAction/queryRoleMenus',{id:data.id},
					function(data){
		...
		//初始化所有菜单
		$("#rights").tree({
			url:'${pageContext.request.contextPath }/web/menuAction/queryAllSysMenus',
			checkbox:true,
			lines:true,
			cascadeCheck:false,
	...
	function add(){
		url="${pageContext.request.contextPath }/web/roleAction/add";
		$('#fm').form('clear');
	...
	function del(){
		...
		if (row == null) {
			...
			$.ajax({
				url:'${pageContext.request.contextPath }/web/roleAction/delete',
				data:{"id":row.id},
	...
	function edit(){
		...
		if (selected == null) {
			...
			var row = $('#roles').datagrid('getSelected');
			url="${pageContext.request.contextPath }/web/roleAction/update?id="+row.id;
			if (row) {
				$('#fm').form('clear');
	...
	function addRight(){
		...
		if (role == null) {
			...
			if(rights!=null){
				$.ajax({
					url:'${pageContext.request.contextPath }/web/menuAction/authorRights',
...
<body class="easyui-layout" fit=true>
	<div data-options="region:'east',title:'分配权限'" style="width:250px;" collapsible=false>
		<ph:rightFilter url="web/menuAction/authorRights">
