...
<script type="text/javascript">
	...
	$(function(){
		$("#users").datagrid({
			url:'${pageContext.request.contextPath }/web/userAction/userInfos',
			idField:'id',
			loadMsg:'请稍后，数据加载中...............',
			pagination:true,
			striped:true,
	...
	function restPwd(){
		...
		if (row == null) {
			...
			$.ajax({
				url:'${pageContext.request.contextPath }/web/userAction/restPassword',
				data:{id:row.id},
	...
	function userAuthor(){
		...
		if (row == null) {
			...
			$.ajax({
				url:'${pageContext.request.contextPath }/web/roleAction/queryUserRoles',
				data:{uId:row.id},
				success:function(data){
					if(data.flag){
						$('#authorFm').form('clear');
	...
	function authorization(){
		$("#authorFm").form("submit",{
			url:'${pageContext.request.contextPath }/web/roleAction/authorization',
			onSubmit:function(){
...
<body>
	...
	<div id="audg" class="easyui-dialog" draggable="false" modal=true
		...
		<form id="authorFm" method="post" novalidate>
			...
			<div class="fitem">
				<label>角色名称:</label> <input name="rIds" class="easyui-combobox" multiple=true data-options="valueField:'id',textField:'roleName',url:'${pageContext.request.contextPath }/web/roleAction/queryAllRoles'">
			</div>
			<div class="fitem">
